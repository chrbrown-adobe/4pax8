...
/* global WebImporter */
/**
 * Parser for cards-vendor. Base: cards.
 ...
 * Each subsequent row is one vendor card: [ logo image cell | text content cell ].
 * Text cell holds vendor name (heading) + description + CTA link.
 *
 * Icon handling: vendor cards use logo PNG <img> images in the image cell — keep as images.
 *
 * Source: Elementor top-level columns. Each card column has a logo image widget followed by
 * an inner .elementor-section holding a heading widget and a text-editor (description + CTA).
 * Only TOP-LEVEL columns are cards; inner-section columns are layout wrappers.
 */
export default function parse(element, { document }) {
  ...
  cardColumns.forEach((col) => {
    ...
    // Text content: vendor name heading + description + CTA.
